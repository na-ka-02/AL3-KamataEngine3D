...
void Player::Initialize(Model* model, ViewProjection* viewProjection, const Vector3& position)
{
	...
	//初期回転
	worldTransform_.rotation_.y = std::numbers::pi_v<float>*2.0f / 2.0f;
}
...
void Player::Update()
{
	...
	else
	{
		...
		{
			//めり込み排斥
			worldTransform_.translation_.y = 1.6f;
			//摩擦で横方向速度が減衰する
			velocity_.x *= (1.0f - kAttenuation);
			//下方向速度をリセット
			velocity_.y = 0.0f;
...
void Player::groundCollision(const CollisionMapInfo& info)
{
	if (onGround_)
	{
		...
		else
		{
			...
			IndexSet indexSet{};
			//左下の判定
			indexSet = mapChipField_->GetMapChipIndexSetByPosition(positionsNew[kLeftBottom] + Vector3(0, -kAttenuationLanding, 0));
			...
			// 右下点の判定
			indexSet = mapChipField_->GetMapChipIndexSetByPosition(positionsNew[kRightBottom] + Vector3(0, -kAttenuationLanding, 0));
			mapChipType = mapChipField_->GetMapChipTypeByIndex(indexSet.xIndex, indexSet.yIndex);
			if (mapChipType == MapChipType::kBlock)
			{
				hit = true;
